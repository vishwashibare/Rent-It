...
module.exports.carformsubmitted = async (req, res) => {
    ...
    const cardataimage = req.files.map(f => ({ url: f.path, filename: f.filename }));
...
module.exports.perticularCar = async(req,res)=>{
    let isrented = false;
    const car = await carschema.findById(req.params.id).populate("author").populate("reviews");
    if(req.user){
    ...
    res.render("./views/car/perticularCar",{car,isrented});
